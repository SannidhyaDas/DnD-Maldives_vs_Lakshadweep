...
---- Digits n Data -------
--SQL Challenge ------------------------------------------
---------------- Maldives vs Lakhsadweep ------ By Sannidhya Das 
...
-- Q1 : Retrieve the number of visitors to the Maldives in 2019?

SELECT name AS destination_name,year,numberofvisitors FROM destination
JOIN touristarrivals ON destination.destinationid=touristarrivals.destinationid
WHERE year=2019 AND name LIKE 'Maldives%';

-- Q2 : Retrieve the average budget per day and best time to visit for 
-- destinations with more than 10,000 tourists in 2022?

SELECT name AS destination_name,averagebudgetperday,besttimetovisit,
numberofvisitors,year FROM destination
JOIN touristarrivals ON destination.destinationid=touristarrivals.destinationid
WHERE year=2022 
GROUP BY name,averagebudgetperday,besttimetovisit,numberofvisitors,year
HAVING numberofvisitors > 10000;

-- Q3: Retrieve the unique features of tourist attractions in Lakshadweep?

SELECT destination.name AS destination_name,
touristattractions.name AS visiting_spot_name, description
FROM destination
JOIN touristattractions ON 
destination.destinationid=touristattractions.destinationid
WHERE destination.name='Lakshadweep';

-- Q4: Retrieve the number of Indian tourists who visited the Maldives in 2021?

SELECT name AS destination_name,numberoftourists,year 
FROM destination
JOIN indiantourists ON destination.destinationid=indiantourists.destinationid
WHERE destination.destinationid=1 AND year=2021
;

-- Q5 : Find the destinations with the highest tourism revenue in 2022?

WITH rev AS (
SELECT name AS destination_name,tourismrevenue,numberofvisitors,year
FROM destination
JOIN touristarrivals ON destination.destinationid=touristarrivals.destinationid
)
SELECT destination_name,
ROUND((tourismrevenue*numberofvisitors),2) AS total_rev,year
FROM rev
WHERE year=2022
ORDER BY 2 DESC
LIMIT 1;
